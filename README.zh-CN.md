...
现在还很早：**2,851 条记录里，有这一页的是 49 个。**其余的先挂着元数据、一条
真实安装出来的结论和简介，等文章补上。没有为了让数字好看去灌生成的水文。

## 收录情况

| | |
| --- | --- |
| 收录总数 | **2,851** —— 未归档的 2,846 |
| 分类 | 14 个，从 Memory、Vision 到 Themes |
| 真装过并有结论 | **2,435** —— 每个一次性容器，装完看 harness 自己的记录 |
| AI 锐评 | 2,270 条，中英双语 |
| 手写中文简介 | 1,005 条，其余暂时只有英文 |
| 写过的详情页 | 49 个，中英双语 · 46 个配图 |
| LINUX DO 认证 | 7 个 |

**真装过并有结论**是这里唯一爬不走的一列：它记的是一次运行，不是一个仓库。
结论分布：`passed` 2,048、`needs-approval` 312、`not-a-layer` 43、`failed` 31、
...
**`github:owner/repo#subpath` 跑不通，但 `#path:` 可以。** 裸写那种确实会被当
git ref 读，报 `Could not resolve <sub> to a commit`——这也是「monorepo 装不了」
这个说法的来源。但 pnpm 的 fragment 是按 `::` 分段的，其中 `path:` 那段就是子
目录，所以 `github:owner/repo#path:sub` 能装。我们实测通过之后才这么写。目前
还有 49 个子目录插件显示「没有一行安装命令」，那是说少了，正在修。
